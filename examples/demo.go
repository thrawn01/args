...
func main() {
	...
	parser.Pos("count", args.Append(), args.Type(int), args.Help("Count the numbers provided"))
	...
	parser.Pos("infile", args.Optional(), args.Type(string), args.Help("Output file, If none supplied reads from stdin"))
	parser.Pos("outfile", args.Optional(), args.Type(string), args.Help("Input file, If none supplied writes to stdout"))
	...
	parser.Opt("--bind", args.Alias("-b"), args.Type(string), args.VarName("ADDRESS"), args.Default("localhost:8080"),
		args.Env("BIND_ADDRESS"), args.Store(&conf.Bind), args.Help("The network interface to bind too"))

	// Unusual option indicators
	parser.Opt("++unusual", args.Alias("+u"), args.Type(bool), args.Default(false),
		args.Help("Unusual option indicator"))
	parser.Opt("-short", args.Alias("-s"), args.Type(bool), args.Default(false),
		args.Help("Short option indicator"))

	// Booleans
	parser.Opt("--debug", args.Alias("-d"), args.Type(bool), args.Default(false),
	...
	parser.Opt("--endpoint", args.Alias("-e"), args.Append(), args.Type(string),
	...
	parser.Opt("--terms", args.Alias("-t"), args.Array(), args.Type(string),
		args.Default([]string{"foo", "bar"}), args.Help("The terms used to search google with"))

	// Give the user choices
	parser.Opt("--choice", args.Alias("-c"), args.Choice(), args.Type(int),
	...
	parser.Opt("--required", args.Alias("-r"), args.Required(), args.Type(string),
		args.Help("This option is required"))

	// Display Help
	parser.Opt("--help", args.Alias("-h"), args.Type(bool), args.Help("Display this help message"))
	...
	list.Opt("filter", args.Alias("-f"), args.Type(bool), args.Help("Filter the list by this thingy"))
